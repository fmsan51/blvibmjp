...
# TODO: あまりに長いので適度に分割したい
  ...
  if (length(rows_ai_started) != 0) {
    ...
    cows_first_ai[, 
                  ':='(day_heat1 = day_heat,
                       day_next_heat = day_heat + heat_cycle(n_cows_first_ai))]
    cows_first_ai[day_next_heat <= 30, day_heat2 := day_next_heat]
    cows_first_ai[day_next_heat <= 30,
                  day_next_heat := day_next_heat + heat_cycle(.N)]
    cows_first_ai[, ':='(day_next_heat = ifelse(day_next_heat - 30 > 0,
                                                day_next_heat - 30, 1),
                         is_heat1_detected = is_heat_detected(n_cows_first_ai),
                         is_heat2_detected = is_heat_detected(n_cows_first_ai))]
    cows_first_ai[, is_ai1_successed := ifelse(
                      is_heat1_detected,
                      is_first_ai_successed(n_cows_first_ai),
                      F)]
    ...
    cows_first_ai[is_heat2_detected == T,
                  is_ai2_successed := ifelse(is_heat1_detected,
                                             is_ai_successed(.N),
                                             is_first_ai_successed(.N))]
    ...
    while (sum(cows_first_ai$n_ai == 0) != 0) {
      cows_first_ai[n_ai == 0,
                    is_heat1_detected := is_heat_detected(.N)]
      cows_first_ai[n_ai == 0,
                    is_ai1_successed := ifelse(is_heat1_detected,
                                               is_first_ai_successed(.N), F)]
      ...
      cows_first_ai[n_ai == 0 & n_heat == 2,
                    is_heat2_detected := is_heat_detected(.N)]
    ...
    cows_first_ai_melt <- melt(cows_first_ai,
    ...
    day_rectal_palpation[seq_len(n_ai_done),
  ...
  if (length(rows_open) != 0) {
    ...
    if (n_cow_heat_missed != 0) {
      day_rectal_palpation[
        (day_rp_last_row + 1): (day_rp_last_row + n_cow_heat_missed),
    ...
    cows_ai[, ':='(day_heat1 = day_heat,
                   day_next_heat = day_heat + heat_cycle(n_cows_ai))]
    cows_ai[day_next_heat <= 30, day_heat2 := day_next_heat]
    cows_ai[day_next_heat <= 30, 
            day_next_heat := day_next_heat + heat_cycle(.N)]
    cows_ai[, ':='(day_next_heat = ifelse(day_next_heat - 30 > 0,
                                          day_next_heat - 30, 1),
                   is_heat1_detected = is_heat_detected(n_cows_ai),
                   is_heat2_detected = is_heat_detected(n_cows_ai))]
    cows_ai[is_heat1_detected == T, is_ai1_successed := is_ai_successed(.N)]
    ...
    cows_ai[is_heat2_detected == T, is_ai2_successed := is_ai_successed(.N)]
    ...
    day_rectal_palpation[day_rp_last_row + seq_len(n_ai_done),
  ...
  if (n_pregnant != 0) {
    day_rectal_palpation[(day_rp_last_row + 1):(day_rp_last_row + n_pregnant),
  ...
  if (n_delivered != 0) {
    day_rectal_palpation[(day_rp_last_row + 1):(day_rp_last_row + n_delivered),
                         ':='(cow_id = cows_delivered$cow_id,
                              infection_status = 
                                cows_delivered$infection_status,
                              day_rp = 
                                15 * (cows_delivered$day_last_heat <= 15) +
  ...
  if (day_rp_last_row != 0) {
    rp_inf_check <- day_rectal_palpation[seq_len(day_rp_last_row), ]
    ...
    rp_inf_check[infection_status == "s",
                 is_infected := (is_after_inf & is_infected_rp(.N))]
    ...
    cows[rows_inf_rp,
         c("date_ipl_expected", "date_ebl_expected") :=
           months_progression(susceptibility_ial_to_ipl,
...
#' @export
change_stage <- function(cows, groups, i) {
  # TODO: groups と barns が入り混じってるのでbarnsに統一したい
  
  # TODO: とりあえず12-23がheifer
  param_group_id <- PARAMS_GROUPS$groups
  ...
  if (params_calculated$is_ts[1]) {
    groups[[1]] <- remove_from_group(groups[[1]], cows[row_c2h, cow_id])
  }
  if (params_calculated$is_ts[2]) {
  ...
  row_m2d <- which(cows$stage == "milking" & 
                     is_dried(i - cows$date_last_delivery))
...
#' @return A [cow_table]
#' @export
change_infection_status <- function(cows, i, month) {
  
  rows_s <- which(cows$infection_status == "s")
  
  is_inf_insects <- is_infected_insects(rows_s, month)
  rows_inf_insects <- rows_s[is_infected_insects(rows_s, month)]
  rows_inf_needles <- rows_s[!(is_inf_insects) & is_infected_needles(rows_s)]
...
#' 
#' @param cows See [cow_table].
#' @param params_calculated See [calculate_parameters].
#' @param i The number of months from the start of the simulation.
#' 
#' @return A [cow_table]
#' @export
add_newborns <- function(cows, params_calculated, i) {
  ...
  if (length(rows_mothers) != 0) {  # If there is any cow delivers in 'month i'
    ...
    newborns[is.na(is_replacement),
             is_replacement := is_replacement(.N, params_calculated$capacity, n_cows)]
    
    # Setting of longevity
    longevity <- longevity(n_newborns)
    ...
    rows_infected_vertical <- is_infected_vertical(newborns$status_mother)
    ...
    newborns <- newborns[!is_stillbirth(parity_mothers), ]
...
#' @export
check_removal <- function(cows, groups, i) {
  # Removal by death
  ...
  rows_removed_ebl <- rows_new_ebl[is_ebl_detected(rows_new_ebl)]
  ...
  if (length(rows_overlooked) != 0) {
    months_until_ebl_die <- months_until_ebl_die(rows_overlooked) 
    cows[seq_len(nrow(cows)) %in% rows_overlooked & 
         date_death_expected >= i + months_until_ebl_die,
         ':='(date_death_expected = i + months_until_ebl_die,
  ...
  for (group in params_calculated$ts_groups) {
...
#' 
#' @param cows See [cow_table].
#' @param params The parameter list used in the simulation. For more detail, see [calc_params()].
...
#' 
#' @rdname save_file
#' @return An output file at the specified location (`save_to_csv()` and `save_params()`) or a character string indicates a file path (eg. "subdir/filename001.csv") (`construct_filepath()`).
#' @export
save_to_csv <- function(cows, filename, i, subdir = NA) {
...
#' @rdname save_file
#' @export
save_params_txt <- function(params, filename, i, subdir) {
  sink(file = construct_filepath(filename, i, subdir, ".txt"))
  str(params, vec.len = Inf, give.attr = F, give.head = F,
...
#' @rdname save_file
#' @export
construct_filepath <- function(filename, i = NULL, subdir = NA, ext = ".csv") {
  if (is.null(i)) {
    filenameXX <- filename
  } else {
    filenameXX <- paste0(filename,
                         formatC(i, width = 2, flag = "0"),  #TODO: i を 4桁に
                         ext)
  }
  if (is.na(subdir)) {
  } else {
    filenameXX <- file.path("data", "output", subdir, filenameXX)
  }
  return(filenameXX)
}
